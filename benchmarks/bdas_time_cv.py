...
from core.utils.env_util import environment_init, split_validation_data, get_bdas_time_cv_args
...
RESULTS_SAVED_PATH = f"bdas_{time_str}_{space}_{K}_total_time_{total_time}_data_{data}_{n_splits}fold"
...
for kth in range(n_splits):
    ...
    cur_fold_data_dir = osp.normpath(osp.join(data_dir, str(kth + 1)))
    if not osp.exists(cur_fold_data_dir):
        os.makedirs(cur_fold_data_dir)
    ...
    controller = ProxyModel(sp, K, saved_path=cur_fold_data_dir, strategy=strategy)
    ...
    while True:
        ...
        for t, action in enumerate(actions, 1):
            ...
            with open(osp.join(cur_fold_data_dir, 'train_history.csv'), mode='a+', newline='') as f:
                current_data = [reward]
