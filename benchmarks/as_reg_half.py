...
from core.utils.env_util import environment_init, get_autosklearn_args
...
args = get_autosklearn_args()
total_time = args.total_time = 14400
id_ = args.data
n_splits = args.n_splits

# create directory to save the result
time_str = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime(time.time()))
RESULTS_SAVED_PATH = f"autosklearn_reg_{time_str}_time_{total_time}_data_{id_}"
...
for kth in range(n_splits):
    ...
    regressor = AutoSklearnRegressor(time_left_for_this_task=total_time, per_run_time_limit=120,
...
logger.info(f"autosklearn on {id_}, {n_splits}-fold cross val score: {cvs}.")
print(f"autosklearn on {id_}, {n_splits}-fold cross val score: {cvs}.")
