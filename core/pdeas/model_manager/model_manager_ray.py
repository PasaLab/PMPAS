...
from sklearn.ensemble import RandomForestClassifier, ExtraTreesClassifier, AdaBoostClassifier, \
    GradientBoostingClassifier, BaggingRegressor
...
from core.utils.constants import PER_MODEL_RUNTIME_LIMIT
...
class Estimator(object):
    def __init__(self, operator_str, cell_time_limit, cv, task, new_x_train, y_train):
        self.operator_str = operator_str
        self.cell_time_limit = cell_time_limit
        self.cv = cv
        self.task = task
        if self.task == 'classification':
            if operator_str == "LogisticRegression":
                self.operator = LogisticRegression()
            if operator_str == "SVC":
                self.operator = SVC(probability=True)
            if operator_str == "RandomForestClassifier":
                self.operator = RandomForestClassifier(n_estimators=500)
            if operator_str == "ExtraTreesClassifier":
                self.operator = ExtraTreesClassifier(n_estimators=500)
            if operator_str == "GaussianProcessClassifier":
                self.operator = GaussianProcessClassifier()
            if operator_str == "AdaBoostClassifier":
                self.operator = AdaBoostClassifier()
            if operator_str == "GradientBoostingClassifier":
                self.operator = GradientBoostingClassifier()
            if operator_str == "MLPClassifier":
                self.operator = MLPClassifier((64, 32))
            if operator_str == "BernoulliNB":
                self.operator = BernoulliNB()
            if operator_str == "KNeighborsClassifier":
                self.operator = KNeighborsClassifier()
            if operator_str == "LinearDiscriminantAnalysis":
                self.operator = LinearDiscriminantAnalysis()
            if operator_str == "QuadraticDiscriminantAnalysis":
                self.operator = QuadraticDiscriminantAnalysis()
            if operator_str == "GaussianNB":
                self.operator = GaussianNB()
            if operator_str == "MultinomialNB":
                self.operator = MultinomialNB()
            if operator_str == "DecisionTreeClassifier":
                self.operator = DecisionTreeClassifier()
            if operator_str == "SGDClassifier":
                self.operator = SGDClassifier(loss="log")
        elif self.task == 'regression':
            if operator_str == 'ARDRegression':
                self.operator = ARDRegression()
            if operator_str == 'AdaBoostRegressor':
                self.operator = AdaBoostRegressor()
            if operator_str == 'BaggingRegressor':
                self.operator = BaggingRegressor()
            if operator_str == 'DecisionTreeRegressor':
                self.operator = DecisionTreeRegressor()
            if operator_str == 'ExtraTreesRegressor':
                self.operator = ExtraTreesRegressor()
            if operator_str == 'GradientBoostingRegressor':
                self.operator = GradientBoostingRegressor()
            if operator_str == 'KNeighborsRegressor':
                self.operator = KNeighborsRegressor()
            if operator_str == 'LinearSVR':
                self.operator = LinearSVR()
            if operator_str == 'MLPRegressor':
                self.operator = MLPRegressor()
            if operator_str == 'NuSVR':
                self.operator = NuSVR()
            if operator_str == 'RandomForestRegressor':
                self.operator = RandomForestRegressor()
            if operator_str == 'Ridge':
                self.operator = Ridge()
            if operator_str == 'SGDRegressor':
                self.operator = SGDRegressor()
            if operator_str == 'XGBRegressor':
                self.operator = XGBRegressor()
            if operator_str == 'XGBRFRegressor':
                self.operator = XGBRFRegressor()
        else:
            raise ValueError("Not supported estimators ...")
        self.new_x_train = new_x_train
        self.y_train = y_train
        self.train_proba = None

    def operator_refit(self):
        return timeout(self.cell_time_limit, exception_message="Time limit for current cell has reached.")(
            self._operator_fit)()

    def operator_fit(self):
        return timeout(self.cell_time_limit, exception_message="Time limit for current cell has reached.")(
            self._operator_fit)()

    def _operator_fit(self):
        self.operator.fit(self.new_x_train, self.y_train)
        if self.task == 'classification':
            train_proba = cross_val_predict(self.operator, self.new_x_train, self.y_train, cv=self.cv,
        ...
        return self.train_proba

    def predict_proba(self, new_x_val):
        if self.task == 'classification':
            return self.operator.predict_proba(new_x_val)
        elif self.task == 'regression':
            return self.operator.predict(new_x_val)
        else:
            raise ValueError("Not supported task.")
...
class Cascade:
    ...
    def __init__(self, child, k_fold=3, max_layers=float("inf"), tolerance=0, early_stopping_rounds=3,
                 random_state=None, cell_time_limit=None, task='classification', confidence_screen=False):
        """
        ...
        if self.task == 'classification':
            self.cv = StratifiedKFold(n_splits=k_fold)
        elif self.task == 'regression':
            self.cv = 3
    ...
    def _fit_cls(self, X, y, X_val=None, y_val=None):
                ...
                for (i, (operator, index)) in enumerate(zip(operators, indexes)):
                    if index == 0:
                        if layer_id == 1:
                            ...
                            new_x_train = np.hstack([x_train, *prev_train_outputs])
                    else:
                        new_x_train = np.hstack([x_train, train_outputs_all[index - 1]])
                    logger.debug(
                    ...
                    if not preserved[i]:
                        ...
                        train_outputs_all.append([])
                        estimators_ray.append(
                            Estimator.remote(operator_strs[i], self.cell_time_limit, self.cv, self.task,
                                             new_x_train, y_train))
                futures = [estimator_ray.operator_fit.remote() for estimator_ray in estimators_ray]
                train_outputs = ray.get(futures)
                prev_train_outputs = train_outputs
                y_train_pred = self.classes.take(np.mean(train_outputs, axis=0).argmax(axis=1))
                layer_score = accuracy_score(y_train, y_train_pred)
                ...
                for (i, (operator, index)) in enumerate(zip(operators, indexes)):
                    if index == 0:
                        if layer_id == 1:
                            ...
                            new_x_val = np.hstack([x_val, *prev_val_outputs])
                    else:
                        new_x_val = np.hstack([x_val, val_outputs_all[index - 1]])
                    logger.debug(
                        f"[layer {layer_id}], validate {operator_strs[i]} on X: {new_x_val.shape}.")
                    # todo remove this line?
                    new_x_val = np.nan_to_num(new_x_val)
                    if not preserved[i]:
                        val_proba = operator.predict_proba(new_x_val)
                        val_outputs_all.append(val_proba)
                    else:
                        val_outputs_all.append([])
                        new_x_val_lists.append(new_x_val)

                futures = [estimators_ray[i].predict_proba.remote(new_x_val_lists[i]) for i in
                           range(len(estimators_ray))]
                val_outputs = ray.get(futures)
                prev_val_outputs = val_outputs
                y_train_pred = self.classes.take(np.mean(train_outputs, axis=0).argmax(axis=1))
                y_val_pred = self.classes.take(np.mean(val_outputs, axis=0).argmax(axis=1))
                train_score = accuracy_score(y_train, y_train_pred)
                layer_score = accuracy_score(y_val, y_val_pred)
    ...
    def _fit_reg(self, X, y, X_val=None, y_val=None):
        logger.info("Confidence screen mechanism off.")
        if X_val is None and y_val is None:
            logger.info(
                "Fit process of cascade model, no validation data passed, training terminated by training score.")
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        if self.confidence_screen and self.task == 'regression':
            raise ValueError("Regression task not support confidence screen mechanism.")

        if self.confidence_screen:
            # cls , cs, ray
            return self.fit_cs(X, y, X_val, y_val)
        else:
            if self.task == 'classification':
                # cls, ray
                return self._fit_cls(X=X, y=y, X_val=X_val, y_val=y_val)
            elif self.task == 'regression':
                # reg, ray
                return self._fit_reg(X=X, y=y, X_val=X_val, y_val=y_val)
            else:
                raise ValueError("Not supported task.")
    ...
    def refit(self, X, y):
        if self.task == 'classification':
            return self._refit_cls(X, y)
        elif self.task == 'regression':
            return self._refit_reg(X, y)
        else:
            raise ValueError("Not supported task.")
            ...
            for (i, (operator, index)) in enumerate(zip(operators, indexes)):
                if index == 0:
                    if layer_id == 1:
                        ...
                        new_X_test = np.hstack([X_test, *prev_test_outputs])
                else:
                    new_X_test = np.hstack([X_test, test_outputs_all[index - 1]])
                logger.debug(
                    f"[layer {layer_id}], predict {operator_strs[i]} on X: {new_X_test.shape}.")
                new_X_test = np.nan_to_num(new_X_test)
                test_proba = operator.predict_proba(new_X_test)
                test_outputs_all.append(test_proba)
                if preserved[i]:
                    test_outputs.append(test_proba)
            prev_test_outputs = test_outputs
        return self.classes.take(np.mean(test_outputs, axis=0).argmax(axis=1))

    def _predict_reg(self, X_test):
        """
         Predict the label of the given un-know samples.
         :param X_test:
         :return:
    ...
    def predict(self, X_test):
        if self.task == 'classification':
            return self._predict_cls(X_test)
        elif self.task == 'regression':
            return self._predict_reg(X_test)
        else:
            raise ValueError("Not supported task.")
...
if __name__ == '__main__':
    ...
    ray.init()
    start = time.time()
    x_train, x_test, y_train, y_test = load_data_by_id(11)
    ...
    model = [0, 'MLPRegressor', 1, 'MLPRegressor', ]
    # model = [0, 'RandomForestClassifier', 0,
    #          'RandomForestClassifier', 1, 'ExtraTreesClassifier',
    #          0, 'ExtraTreesClassifier']
    # c = Model(child, model_time_limit=0.001)
    c = Cascade(model)
    logger.info("Begin training...")
    # c.fit_cs(X_train, Y_train, X_val, Y_val)
    c.fit(X_train, Y_train)
    logger.info("Begin refitting...")
    c.refit(x_train, y_train)
    logger.info("Begin testing...")
    end = time.time()
    logger.info(accuracy_score(y_test, c.predict(x_test)))
