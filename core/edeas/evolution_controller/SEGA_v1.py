    ...
    def draw(self, pop, EndFlag=False):
        if not EndFlag:
            ...
            if self.drawing != 0:
                ...
                ea.trcplot(metric, [['population average aimFunc value', 'population best aimFunc value']],
...
class soea_SEGA_templet(ea.SoeaAlgorithm):
    ...
    def __init__(self, problem, population, result_path='', stop_by_time=False):
        ea.SoeaAlgorithm.__init__(self, problem, population)
        if population.ChromNum != 1:
            raise RuntimeError('Must pass single DNA type.')
        self.name = 'SEGA'
        self.selFunc = 'tour'
